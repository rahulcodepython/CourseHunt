...
const bubbleVariants = cva(
  "relative max-w-[85%] rounded-2xl px-4 py-2.5 text-sm transition-colors",
  {
    variants: {
      variant: {
        default:
          "bg-primary text-primary-foreground rounded-bottom-left-xs",
        secondary:
          "bg-secondary text-secondary-foreground rounded-bottom-left-xs",
        muted:
          "bg-muted text-muted-foreground border rounded-top-left-xs",
        outline:
          "border bg-background text-foreground shadow-xs rounded-top-left-xs",
      },
    },
    defaultVariants: {
      variant: "outline",
...
function BubbleHeader({
  ...
  return (
    <div
      ...
      className={cn("mb-1 flex items-center justify-between gap-2 text-xs font-medium text-muted-foreground", className)}
      {...props}
    />
  );
}
