...
func (m *CoursesModule) ListRepository(page, limit int, userID string, scope generic.AuthScope, categoryID, subcategoryID, level, search, status, filterTutorID string) ([]Course, int, error) {
	offset := (page - 1) * limit
	where := []string{}
	args := []interface{}{}
	idx := 1
