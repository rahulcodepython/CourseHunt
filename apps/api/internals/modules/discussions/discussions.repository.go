...
func buildUpdateAuthCTE(scope generic.AuthScope) string {
	...
	default:
		return `auth AS (SELECT true AS is_authorized),`
	}
}
...
func (m *DiscussionsModule) ListRepository(lessonID, parentID, userID string, scope generic.AuthScope, page, limit int) ([]Discussion, int, error) {
...
func (m *DiscussionsModule) CreateRepository(userID string, req CreateDiscussionRequest, scope generic.AuthScope) (*Discussion, error) {
...
func (m *DiscussionsModule) UpdateRepository(id, userID string, content string, scope generic.AuthScope) (*Discussion, error) {
	...
	query := fmt.Sprintf(`
		...
		updated AS (
			...
			CROSS JOIN auth a
			WHERE discussions.id = $1 AND a.is_authorized = true %s
	...
	`, authCTE, ownerClause,
		map[bool]string{true: "EXISTS(SELECT 1 FROM discussion_info WHERE user_id = $2)", false: "true"}[checkOwner],
...
func (m *DiscussionsModule) DeleteRepository(id, userID string, scope generic.AuthScope) (string, error) {
