...
func (m *DiscussionsModule) TutorListRepository(lessonID, parentID, tutorID string, page, limit int) ([]Discussion, int, error) {
	...
	err := m.DB.Get(&result, query, lessonID, parentID, tutorID, limit, offset)
	if err != nil {
		return nil, 0, err
	}
...
func (m *DiscussionsModule) TutorCreateRepository(tutorID string, req CreateDiscussionRequest) (*Discussion, error) {
	var result struct {
	...
	err := m.DB.Get(&result, query, req.LessonID, req.ParentID, tutorID, req.Content)
	if err != nil {
		return nil, err
	}
...
func (m *DiscussionsModule) TutorUpdateRepository(id, tutorID string, content string) (*Discussion, error) {
	var result struct {
	...
	query := `
		...
		updated AS (
			...
			WHERE discussions.id = $1 AND discussions.user_id = $2
	...
	err := m.DB.Get(&result, query, id, tutorID, content)
	if err != nil {
		return nil, err
	}
