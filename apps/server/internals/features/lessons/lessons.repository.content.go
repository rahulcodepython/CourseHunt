...
func (a *App) StudentReadContentRepository(ctx context.Context, lessonID, userID string) (*AggregatedLessonContentResponse, error) {
	...
	err := a.DB.QueryRow(ctx, ReadContentStudent, lessonID, userID).Scan(&lessonExists, &isEnrolled, &isLocked, &contentData)
	if err != nil {
		return nil, postgres.MapPgError(err)
	}
	...
	if err := postgres.CheckConditions(
		...
		postgres.Condition{Failed: isLocked, Err: generic.ErrLessonsChapterLocked},
...
func (a *App) RecordHeartbeatRepository(ctx context.Context, lessonID, userID string, req HeartbeatRequest) (*HeartbeatResponse, error) {
	var (
		lessonExists bool
		isEnrolled   bool
		currentStreak int
	...
	err := a.DB.QueryRow(ctx, RecordHeartbeat, lessonID, userID, req.PlaybackSeconds, req.SessionSeconds).Scan(
		&lessonExists,
		&isEnrolled,
		&currentStreak,
		&totalMinutes,
	...
	return &HeartbeatResponse{
		LessonID:        lessonID,
		PlaybackSeconds: req.PlaybackSeconds,
