...
// server's lifecycle context and bounded by a timeout. It sets this context
// via c.SetUserContext(ctx).
//
// Any downstream database queries (pgx), cache commands (redis), or outbound
// HTTP requests that use c.UserContext() will automatically receive cancellation
// if:
