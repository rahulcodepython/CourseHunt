...
func NewChaptersRepository(db *sqlx.DB, coursesRepo *CoursesRepository, cache *cache.Cache) *ChaptersRepository {
	return &ChaptersRepository{DB: db, CoursesRepo: coursesRepo, Cache: cache}
}

// Explicit, granular domain errors

func (r *ChaptersRepository) ListRepository(courseID, userID string, scope generic.AuthScope) ([]entities.Chapter, error) {
	switch scope {
	...
	default:
		...
		case 0:
			return nil, generic.ErrChaptersCourseNotFound
		case 1:
			return nil, generic.ErrChaptersUnauthorized
		default:
			var chapters []entities.Chapter
...
func (r *ChaptersRepository) CreateRepository(userID, courseID string, req entities.CreateChapterRequest) (*entities.Chapter, error) {
	query := `
...
func (r *ChaptersRepository) UpdateRepository(id, userID string, req entities.UpdateChapterRequest) (*entities.Chapter, error) {
	query := `
		WITH status_check AS (
			SELECT
				CASE
	...
	switch res.StatusFlag {
	case 0:
		return nil, generic.ErrChaptersChapterNotFound
	case 1:
		return nil, generic.ErrChaptersUnauthorized
	default:
		var chapter entities.Chapter
...
func (r *ChaptersRepository) DeleteRepository(id, userID string) (string, error) {
	query := `
		WITH status_check AS (
			SELECT
				CASE
	...
	switch res.StatusFlag {
	case 0:
		return "", generic.ErrChaptersChapterNotFound
	case 1:
		return "", generic.ErrChaptersUnauthorized
	default:
