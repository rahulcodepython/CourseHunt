            ...
            <div className="bg-linear-to-br from-primary/10 via-background to-secondary/10 py-12">
                <div className="container mx-auto grid gap-8 px-4 lg:grid-cols-3">
                    <div className="space-y-4 lg:col-span-2">
                        ...
                        {course.short_description && <p className="text-xl text-muted-foreground">{course.short_description}</p>}
                        ...
                        <div className="flex items-center gap-3 pt-2">
                            <UserAvatar name={course.instructor.name} image={course.instructor.image} className="size-12" />
                            <div>
                                ...
                                {course.is_free ? (
                                    ...
                                    <div className="flex items-baseline gap-2">
                                        ...
                                        {course.final_price < course.actual_price && (
                                            <span className="text-muted-foreground line-through">{formatINR(course.actual_price)}</span>
                                        )}
                                    </div>
                                )}
                                            ...
                                            <AccordionTrigger className="px-3">
                                                <div className="text-left">
                                                    ...
                                                    <p className="text-xs text-muted-foreground">
                                                        {chapter.lessons.length} lectures &middot; {formatDuration(chapter.total_duration_seconds)}
                                                    </p>
                                                </div>
                                            </AccordionTrigger>
                                            <AccordionContent className="px-3">
                                                <div className="space-y-1 border-l-2 border-muted pl-4">
                                                    {chapter.lessons.map((lesson) => (
                                                        <div key={lesson.id} className="flex items-center justify-between gap-3 py-1.5 text-sm">
                                                            <div className="flex items-center gap-2 text-muted-foreground">
                                                                <Icon name={LESSON_TYPE_ICON[lesson.lesson_type] ?? "file-text"} className="size-4" />
                                                                {lesson.title}
                                                            </div>
