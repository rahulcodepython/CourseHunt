...
function PermissionsGrid({
  ...
  const tutorItems = rawPermissions
    .filter((p) => p.id.startsWith("tutor:"))
    .map((p) => ({ id: p.id, label: p.name }));

  const otherItems = rawPermissions
    .filter((p) => !p.id.startsWith("admin:") && !p.id.startsWith("tutor:"))
    .map((p) => ({ id: p.id, label: p.name }));

  const groups = [];

  if (adminItems.length > 0) groups.push({ groupName: "Admin", items: adminItems });
  if (tutorItems.length > 0) groups.push({ groupName: "Tutor", items: tutorItems });
  if (otherItems.length > 0) groups.push({ groupName: "General / Other", items: otherItems });
    ...
    >
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        ...
        <div className="space-y-1.5">
          ...
          <Input id="role-desc" placeholder="What can this role do?" {...register("description")} />
        </div>
        <DialogFooter>
