...
function LessonRow({
    ...
    return (
        ...
        >
            ...
            <Icon name={LESSON_TYPE_ICON[lesson.lesson_type] ?? "file-text"} className="size-4 shrink-0 text-muted-foreground" />
...
export function CourseSidebar({
    ...
    const [expanded, setExpanded] = React.useState<string[]>(activeChapterId ? [activeChapterId] : []);
                                ...
                                <AccordionTrigger className="min-w-0 px-2.5 py-2.5 hover:no-underline">
                                    <div className="min-w-0 flex-1 text-left">
                                        ...
                                        <p className="mt-0.5 text-xs text-muted-foreground">
                                            {chapter.progress.lessons_completed}/{chapter.total_lectures} lectures &middot; {formatDuration(chapter.total_duration_seconds)}
                                        </p>
                                    </div>
                                </AccordionTrigger>
                                <AccordionContent className="pb-1">
                                    <div className="space-y-0.5">
                                        {chapter.lessons.map((lesson) => (
                                            <LessonRow key={lesson.id} courseId={courseId} lesson={lesson} active={lesson.id === activeLessonId} />
                                        ))}
                                    </div>
                                </AccordionContent>
                            </AccordionItem>
