...
export function buildSubmitRequest(answers: AnswerDraft[]): SubmitQuizRequest {
  return {
    single_answers: answers
      .filter(
        (a): a is Extract<AnswerDraft, { type: "single_choice" }> => a.type === "single_choice",
      )
      .map(({ question_id, selected_option_id, is_skipped }) => ({
        question_id,
        selected_option_id,
        is_skipped,
      })),
    multi_answers: answers
      ...
      .map(({ question_id, selected_option_ids, is_skipped }) => ({
        question_id,
        selected_option_ids,
        is_skipped,
      })),
