  ...
  const columns: ColumnDef<QuizAttemptSummary, any>[] = React.useMemo(
    () => [
        ...
        cell: ({ getValue }) => {
          ...
          return (
            <span className="text-muted-foreground">
              {value ? formatDateTime(value) : "In progress"}
            </span>
          );
        },
      }),
      columnHelper.accessor("total_score", {
        header: "Score",
        cell: ({ getValue }) => (
          <span className="font-medium tabular-nums">{Math.round(getValue())}%</span>
        ),
      }),
      columnHelper.accessor(
        (row) =>
          `${row.correct_count}/${row.correct_count + row.incorrect_count + row.skipped_count}`,
        {
          id: "correct",
          header: "Correct",
          cell: ({ getValue }) => (
      ...
      columnHelper.accessor("passed", {
        header: "Result",
        cell: ({ getValue }) => (
          <StatusBadge status={getValue() ? "passed" : "failed"} map={statusMap} />
        ),
      }),
        ...
        cell: ({ row }) => (
          <div className="flex justify-end">
            <Button
              variant="outline"
              size="sm"
              onClick={() => setSelectedAttemptId(row.original.id)}
            >
              View Breakdown
            </Button>
          </div>
