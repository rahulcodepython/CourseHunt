...
export default function StudyPage() {
    ...
    const allLessons = React.useMemo(
        () => study?.chapters.flatMap((c) => c.lessons) ?? [],
        [study],
    );
        ...
        return (
            <Card>
                <CardContent className="flex flex-col items-center gap-3 py-16 text-center">
                    ...
                    <p className="text-sm text-muted-foreground">Select a lesson from the sidebar to get started.</p>
                </CardContent>
            </Card>
        );
    }
            ...
            <Card>
                <CardHeader className="flex flex-row items-center justify-between gap-4">
                    ...
                    <div className="flex items-center justify-between gap-2">
                        <Button variant="outline" size="sm" disabled={!previousLesson} asChild={!!previousLesson}>
                            {
                                previousLesson ? <Link href={`/student/study/${courseId}?lessonId=${previousLesson.id}`}>
                        ...
                        <Button variant="outline" size="sm" disabled={!nextLesson} asChild={!!nextLesson}>
                            {
                                nextLesson ? <Link href={`/student/study/${courseId}?lessonId=${nextLesson.id}`}>
                                    Next Lesson
                ...
                <CardContent>
                    {content.lesson_type === LESSON_TYPE.VIDEO && content.video_content && (
                        <LessonVideoPlayer src={content.video_content.video_url} title={lessonMeta?.title ?? "Lesson"} />
                    )}
