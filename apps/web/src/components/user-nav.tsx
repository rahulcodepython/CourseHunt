    ...
    return (
        <DropdownMenu>
            <DropdownMenuTrigger asChild>
                <Button variant="ghost" className="relative flex items-center gap-2 rounded-full h-9 px-2 focus:outline-none hover:bg-accent">
                    <UserAvatar name={user?.name ?? "Admin"} image={user?.image} className="size-7 border" />
                    <span className="hidden text-sm font-medium sm:inline-block pr-1">{user?.name ?? "Admin"}</span>
                </Button>
            </DropdownMenuTrigger>
            <DropdownMenuContent side="bottom" align="end" className="w-56">
                <DropdownMenuLabel className="flex flex-col gap-0.5">
                    <span className="font-medium">{user?.name ?? "Admin"}</span>
                    <span className="text-xs font-normal text-muted-foreground">{user?.email ?? "Platform Administrator"}</span>
                </DropdownMenuLabel>
                <DropdownMenuSeparator />
