            ...
            items: group.items
                .map((item): NavItem | null => {
                    if (item.children?.length) {
                        const children = item.children.filter((child) => hasPermission(userPermissions, child.permission));
...
export function isRouteAllowed(pathname: string, routeMap: RoutePermissionMap, userPermissions: string[]): boolean {
