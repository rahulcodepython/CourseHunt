...
export function useTransactionsQuery(params?: { page?: number; limit?: number }) {
	...
	const url = qs ? `${API_ENDPOINTS.TRANSACTIONS}?${qs}` : API_ENDPOINTS.TRANSACTIONS;
...
export function useCheckoutCourseQuery(courseId: string) {
	return useAppQuery(queryKeys.transactionsCheckout(courseId), () =>
		apiRequest({ url: `${API_ENDPOINTS.TRANSACTIONS}/checkout/course/${courseId}`, method: "GET" }, CheckoutCourseResponseZod),
	);
}
...
export function useTransactionStatusQuery(txId: string, options?: { enabled?: boolean; refetchInterval?: number | false }) {
	return useAppQuery(
		...
		() => apiRequest({ url: `${API_ENDPOINTS.TRANSACTIONS}/${txId}/status`, method: "GET" }, TransactionStatusResponseZod),
		options as any,
	);
}
...
export function useInitiateTransactionMutation() {
	return useSimpleMutation({
		mutationFn: (data: z.infer<typeof InitiateTransactionRequestZod>) =>
			apiRequest({ url: API_ENDPOINTS.TRANSACTIONS_INITIATE, method: "POST", data }, InitiateTransactionResponseZod),
		showToast: false,
	});
}
